...
function click(action){const button={dataset:{action},parentElement:app};app.handler({target:{dataset:{},parentElement:button}});}
assert.match(app.innerHTML,/급식동에 데려갈 네 명/);assert.match(app.innerHTML,/CURRENT UNIT/);
click("toggle:hwayoung");assert.match(app.innerHTML,/CURRENT UNIT · 3\/4/);click("toggle:epi-minos");assert.match(app.innerHTML,/CURRENT UNIT · 4\/4/);assert.match(app.innerHTML,/화영/);
click("briefing");assert.match(app.innerHTML,/BRIEFING/);assert.match(app.innerHTML,/제 선 안에서만 움직이세요/);
...
click("start");assert.match(app.innerHTML,/김우주/);assert.match(app.innerHTML,/11\/11 HP/);assert.match(app.innerHTML,/사건 전체 2\/2/);
