...
const fs=require("node:fs"),path=require("node:path"),css=fs.readFileSync(path.join(__dirname,"../styles.css"),"utf8"),cafeteriaApp=fs.readFileSync(path.join(__dirname,"../cafeteria-app.js"),"utf8"),commonUi=fs.readFileSync(path.join(__dirname,"../ui-common.js"),"utf8");
...
assert.match(commonUi,/previous\?\.type==="effect"/,"공통 UI가 연속 effect를 결과 묶음으로 합쳐야 한다.");
assert.doesNotMatch(commonUi,/setTimeout\(/,"플레이 로그는 자동 진행 타이머를 사용하지 않아야 한다.");
assert.match(commonUi,/data-action="presentation-next">계속/,"모든 플레이 로그에 계속 버튼이 있어야 한다.");
