...
window.GameEngine = (() => {
  ...
  function startBriefing(s){if(s.partyIds.length!==4)return false;s.phase="briefing";effect(s,`출전 4명 · 후방 지원 ${character(absentId(s)).name}.`,{stat:"formation"});return true;}
  ...
  function investigate(s,choiceId){
    ...
    const investigator=character(choice.owner);narration(s,`${location.label}에서 ${investigator.name}의 조사가 시작된다.`,{accent:investigator.id});dialogue(s,investigator.voice.explore,investigator.name,{accent:investigator.id});narration(s,choice.result,{accent:investigator.id});effect(s,`단서 확보 — ${event.clueInfo[choice.clue].label}: ${event.clueInfo[choice.clue].effect}`,{source:investigator.id,stat:"clue"});return true;
  }
  function beginStage(s,index){
    ...
    if(index===1&&s.clues.includes("rhythm")){stabilize(s,1);effect(s,"호흡의 빈틈을 이용해 진입 직후 고착을 한 단계 느슨하게 했다.",{stat:"clueBonus"});}
  ...
  function lowerPressure(s,amount){const before=s.threatPressure;s.threatPressure=Math.max(0,s.threatPressure-amount);effect(s,`현상 압력 ${before} → ${s.threatPressure}.`,{stat:"pressure",delta:s.threatPressure-before,tone:"recover"});}
  function stabilize(s,amount=1){const i=worstIndex(s),target=s.students[i],before=target.stage;target.stage=Math.max(0,target.stage-amount);effect(s,`${target.label}: ${event.studentStages[before]} → ${event.studentStages[target.stage]}.`,{source:target.id,stat:"studentState",delta:target.stage-before,tone:"recover"});}
  ...
  function checkFailure(s){if(s.students.some(x=>!x.evacuated&&x.stage>=event.rules.dangerStage)){s.phase="failure";effect(s,"보호 대상이 위험 상태에 도달해 대응을 중단했다.",{stat:"failure",tone:"danger"});}else if(aliveIds(s).length===0){s.phase="failure";effect(s,"출전 인원이 모두 전투 불능이 되어 후퇴했다.",{stat:"failure",tone:"danger"});}}
  function endRound(s){
    s.lastWorsened=null;
    if(s.threatPressure>=event.rules.pressureEscalateAt){
      if(s.supportShield){s.supportShield=false;narration(s,"복도 바깥에서 고정된 문이 현상 반동을 받아내며 크게 울린다.",{accent:"hwayoung",tone:"guard"});effect(s,"화영의 후방 방호가 현상 반동을 1회 차단했다.",{source:"hwayoung",stat:"block",tone:"guard"});}
      else{const activeHwa=s.partyIds.includes("hwayoung")&&member(s,"hwayoung").guard>0,exposed=aliveIds(s);exposed.forEach(id=>{if(!(activeHwa&&id!=="hwayoung"))member(s,id).hp=Math.max(0,member(s,id).hp-1);});narration(s,activeHwa?"화영이 충격선 안으로 한 걸음 들어가 파동을 어깨로 받아낸다.":"교실 전체를 훑은 역류가 출전 인원의 호흡을 끊는다.",{accent:activeHwa?"hwayoung":undefined,tone:"damage"});effect(s,activeHwa?"화영이 현상 반동을 집중해서 받아 HP 1을 잃었다.":"현상 반동으로 출전 인원 전원이 HP 1을 잃었다.",{source:activeHwa?"hwayoung":undefined,stat:"hp",delta:-1,tone:"damage"});exposed.filter(id=>member(s,id).hp===0).forEach(id=>effect(s,`${subject(character(id).name)} 전투 불능이 됐다.`,{source:id,stat:"incapacitated",tone:"danger"}));if(activeHwa){const h=member(s,"hwayoung");h.guard--;h.heat=Math.min(4,h.heat+1);dialogue(s,character("hwayoung").voice.hit,"화영",{accent:"hwayoung"});}else{const reactor=character(s.turnOrder[(s.round-1)%s.turnOrder.length]);dialogue(s,reactor.voice.hit,reactor.name,{accent:reactor.id});}if(s.stageIndex>0&&eligibleStudentIndexes(s).length){const i=worstIndex(s);s.students[i].stage++;s.lastWorsened={stageIndex:s.stageIndex,studentIndex:i};effect(s,`${s.students[i].label}이 ${event.studentStages[s.students[i].stage]} 상태로 악화됐다.`,{source:s.students[i].id,stat:"studentState",delta:1,tone:"danger"});const reactor=character(s.turnOrder[s.round%s.turnOrder.length]);dialogue(s,reactor.voice.worsen,reactor.name,{accent:reactor.id});}}
    }
    if(s.deferredPressure){s.threatPressure+=s.deferredPressure;effect(s,`축적된 후속 반동으로 현상 압력이 ${s.threatPressure}이 됐다.`,{stat:"pressure",delta:s.deferredPressure,tone:"warning"});s.deferredPressure=0;}
  ...
  function evacuate(s,index){const student=s.students[index];if(!student||student.evacuated||student.stage>0)return false;let risk=student.exitRisk;if(s.routeUses>0&&risk>0){risk=Math.max(0,risk-1);s.routeUses--;effect(s,`안전 동선으로 이탈 반동을 줄였다. 남은 안전 통과 ${s.routeUses}회.`,{stat:"routeUses",delta:-1});}student.evacuated=true;s.stageProgress++;s.threatPressure+=risk;narration(s,`${student.label}이 비상계단의 초록선을 따라 교실을 빠져나간다.`,{tone:"success"});effect(s,`${student.label} 이탈 완료. 현상 압력 +${risk}, 남은 학생 ${s.students.filter(x=>!x.evacuated).length}명.`,{source:student.id,stat:"pressure",delta:risk,tone:risk?"warning":"success"});return true;}
  ...
  function applyPhenomenonReaction(s,id,actor){
    ...
    if(s.lastCategory===category&&category!=="other"){s.threatPressure++;narration(s,"같은 대응의 틈을 외운 현상이 한 박자 먼저 움직인다.",{tone:"danger"});effect(s,`연속된 ${category==="protect"?"보호":category==="pressure"?"압력 대응":category==="resource"?"자원 준비":category==="evacuate"?"이탈":"진행"} 방식이 학습됐다. 현상 압력 +1.`,{stat:"pressure",delta:1,tone:"danger"});}
    if(current.id==="recitation"&&["progress","hybrid"].includes(category)){s.threatPressure++;effect(s,"복창 증폭이 목표 개입을 따라붙었다. 현상 압력 +1.",{stat:"pressure",delta:1,tone:"warning"});}
    if(current.id==="fixation"&&category==="protect"){s.threatPressure++;effect(s,"시선 고정이 보호 행동자를 포착했다. 현상 압력 +1.",{stat:"pressure",delta:1,tone:"warning"});}
    if(current.id==="counterpulse"&&["pressure","hybrid"].includes(category)){member(s,actor.id).hp=Math.max(0,member(s,actor.id).hp-1);effect(s,`역류 반동으로 ${subject(actor.name)} HP 1을 잃었다. (${member(s,actor.id).hp} HP)`,{source:actor.id,stat:"hp",delta:-1,tone:"damage"});if(member(s,actor.id).hp===0)effect(s,`${subject(actor.name)} 전투 불능이 됐다.`,{source:actor.id,stat:"incapacitated",tone:"danger"});}
  ...
  function act(s,id){
    ...
    if(id==="steady")stabilize(s);if(id==="intercept"){lowerPressure(s,2);if(s.stageIndex===1&&s.students.some(x=>x.stage>0))stabilize(s);m.guard=1;m.strain=2;}if(id==="burst"){m.heat-=2;lowerPressure(s,4);s.deferredPressure++;effect(s,"방출한 열량이 다음 라운드에 현상 압력 1로 되돌아온다.",{source:c.id,stat:"deferredPressure",delta:1,tone:"warning"});}if(id==="collect"){const gained=s.clues.includes("reports")?2:1;m.buzz+=gained;effect(s,`제보 ${gained} 확보. 현재 ${m.buzz}.`,{source:c.id,stat:"buzz",delta:gained});}if(id==="topic"){m.buzz-=3;lowerPressure(s,3);addProgress(s);if(s.stageIndex===1&&s.students.some(x=>x.stage>0))stabilize(s);m.hp=Math.max(0,m.hp-1);m.overheat=2;effect(s,`열기 역류로 강운심이 HP 1을 잃었다. (${m.hp} HP)`,{source:c.id,stat:"hp",delta:-1,tone:"damage"});if(m.hp===0)effect(s,"강운심이 전투 불능이 됐다.",{source:c.id,stat:"incapacitated",tone:"danger"});}if(id==="archive"){m.memory++;effect(s,`보존 장면 ${m.memory} 확보.`,{source:c.id,stat:"memory",delta:1});}if(id==="recover"){m.memory-=2;if(s.stageIndex>0)stabilize(s);addProgress(s,s.clues.includes("record")?2:1);s.threatPressure++;effect(s,`공허한 수집의 반동으로 현상 압력이 ${s.threatPressure}이 됐다.`,{source:c.id,stat:"pressure",delta:1,tone:"warning"});}if(id==="calm"){if(s.stageIndex===0)lowerPressure(s,2);else stabilize(s,2);m.sedation=2;}if(id==="care")lowerPressure(s,1);if(id==="scan"){if(s.stageIndex===2)lowerPressure(s,2);else{addProgress(s);if(s.stageIndex===1&&s.students.some(x=>x.stage>0))stabilize(s);}m.meltdown=2;}if(id==="patch"){if(s.stageIndex===2){s.routeUses++;effect(s,`안전 통과 1회 확보. 현재 ${s.routeUses}.`,{source:c.id,stat:"routeUses",delta:1});lowerPressure(s,2);}else{lowerPressure(s,3);addProgress(s);}m.meltdown=2;}
  ...
  function useSupport(s){
    ...
    if(id==="hwayoung"){s.supportShield=true;effect(s,"다음 현상 반동을 화영의 후방 방호가 차단한다.",{source:id,stat:"supportShield",tone:"guard"});}if(id==="kang-unshim"){if(s.stageIndex<2)addProgress(s);else{s.routeUses++;effect(s,`안전 통과 1회 확보. 현재 ${s.routeUses}.`,{source:id,stat:"routeUses",delta:1});}s.threatPressure++;effect(s,`화제 확산의 반동으로 현상 압력이 ${s.threatPressure}이 됐다.`,{source:id,stat:"pressure",delta:1,tone:"warning"});}if(id==="epi-minos"){if(s.lastWorsened&&s.lastWorsened.stageIndex===s.stageIndex){const target=s.students[s.lastWorsened.studentIndex];target.stage=Math.max(0,target.stage-1);effect(s,`${target.label}의 직전 악화를 되감았다.`,{source:id,stat:"studentState",delta:-1,tone:"recover"});s.lastWorsened=null;}else if(s.stageIndex<2)addProgress(s);else{s.routeUses++;effect(s,`안전 통과 1회 확보. 현재 ${s.routeUses}.`,{source:id,stat:"routeUses",delta:1});}}if(id==="inan"){if(s.stageIndex>0&&s.students.some(x=>!x.evacuated&&x.stage>0))stabilize(s);else lowerPressure(s,1);}if(id==="kim-wooju"){lowerPressure(s,2);if(s.stageIndex===2){s.routeUses++;effect(s,`안전 통과 1회 확보. 현재 ${s.routeUses}.`,{source:id,stat:"routeUses",delta:1});}}finishStageIfReady(s);return true;
  }
