...
window.SEONGA_LIBRARY_EVENT={
  ...
  factEffectRules:{
    ...
    exitRoute:{stateKey:"routeRisk",initial:1,rewriteIncrease:1,description:"가짜 출구가 현장 재조사를 한 차례 더 붙든다."},
    origin:{stateKey:"correctionBacklash",initial:0,rewriteIncrease:1,lockLoad:1,description:"원인을 바로잡을 때마다 미나의 호흡과 홍채가 흔들린다."}
  },
  facts:{
  ...
  relationshipInteractions:[
    ...
    {id:"unshim-mageuna-cross-examination",category:"comparison",left:{sourceCharacter:"kang-unshim",evidenceKind:"testimony"},right:{sourceCharacter:"mageuna",evidenceKind:"record"},title:"적대적 교차검증",prompt:"운심의 증언 자료와 마근아의 공식 기록이 같은 사실을 두고 충돌한다.",applyLabel:"둘에게 직접 대조시킨다",applyDescription:"공통 사실 판단 가능 · 현실 안정도 -1",skipLabel:"각자 기록만 보존한다",resolver:"unlock-common-judgment"}
  ],
  investigations:[
    {id:"access-terminal",label:"도서관 출입 기록 단말기",prompt:"출입 인원은 셋과 다섯 사이를 오가고, CCTV 시각도 서로 맞지 않는다.",choices:[
      {id:"access-wooju",owner:"kim-wooju",label:"출입 로그와 CCTV 타임스탬프 대조",evidence:"access-log",secondary:"system-difference",approach:"우주가 단말기의 현재 표시와 덮어쓰기 전 원본 로그를 분리한다.",expectation:"인원 대조 · 원본 격리 가능",result:"오프라인 캐시에는 세 개의 출입 서명이, 현재 화면에는 다섯 명 표시가 남았다.",quote:"원본 서명 세 개. 현재 화면 다섯. 미나 선생님, 제 캐시까지 건드리지는 마세요."},
    ...
    {id:"counter",label:"대출대와 반납함",prompt:"대출 기록과 눈앞의 줄이 서로 다른 이용자 수를 주장한다.",choices:[
      {id:"counter-epi",owner:"epi-minos",label:"직전 대출대 장면을 보존",evidence:"counter-memory",approach:"에피가 현재 장면 아래 남은 오염 전 대출대 배치를 카오에게 저장한다.",expectation:"F1 고정 자료",result:"현재 장면 아래에서 이용자 셋이 서 있던 이전 배치가 온전히 분리됐다.",quote:"기록물 복제 아니야. 없어지기 전에 내가 가진 것뿐이야."},
      {id:"counter-mageuna",owner:"mageuna",label:"공식 대출·반납 기록 대조",evidence:"official-record",approach:"마근아가 수정 이력이 남은 화면 대신 직인이 찍힌 마감 기록을 확인한다.",expectation:"F1 고정 자료 · 공식 기준점 가능",result:"마감 기록에는 이용자 셋과 북측 피난 기준이 수정 전 문서로 남아 있었다.",quote:"승인된 업무 기록을 따르겠습니다. 검색 화면은 잠시 덮어두십시오."}
    ]},
    {id:"exit",label:"비상구와 서가 사이 통로",prompt:"서쪽의 새 문은 출구 표지를 달고 있지만 바람도 계단 소리도 통하지 않는다.",choices:[
      {id:"exit-hwayoung",owner:"hwayoung",label:"실제 동선을 직접 통과해 확인",evidence:"physical-route",secondary:"reduce-route-risk",approach:"화영이 양쪽 통로의 압력과 구조를 몸으로 확인해 실제 외부 계단을 찾는다.",expectation:"F2 고정 자료 · 동선 위험 감소",result:"북측에서는 외기와 계단 하중이, 서측에서는 벽면 반사음이 확인됐다.",quote:"서쪽 문은 밀어도 벽의 감촉뿐이오. 북쪽은 소인의 무게를 제대로 받는구려."},
      {id:"exit-mageuna",owner:"mageuna",label:"시설 규정과 피난도를 현장 대조",evidence:"evacuation-rule",approach:"마근아가 학교 시설도와 비상 유도등의 승인 번호를 대조한다.",expectation:"F2 고정 자료 · 공식 기준점 가능",result:"승인 번호가 일치하는 피난도는 북측 비상구 하나만 가리켰다.",quote:"검색 결과를 피난 규정보다 우선하지 마십시오."}
    ]},
    {id:"search-terminal",label:"도서관 검색 단말기",prompt:"같은 검색어가 입력될 때마다 결과의 위치와 작성 시각이 달라진다.",choices:[
      {id:"search-wooju",owner:"kim-wooju",label:"검색 인덱스와 원본 DB 대조",evidence:"index-mismatch",secondary:"system-difference",approach:"우주가 검색 인덱스의 생성 시각을 원본 DB와 나란히 놓는다.",expectation:"동선·발생 순서 대조",result:"현재 검색 결과의 생성 시각은 서가 이동 뒤이며 원본 DB 수정 시각과 어긋나 있었다.",quote:"이 파일, 사고 뒤에 생겼어요. 앞에 끼어들지 마세요. 시간순서는 안 바뀝니다."},
    ...
    {id:"mina-desk",label:"미나의 사서 데스크",prompt:"미나는 정상 운영을 반복하지만 목소리의 노이즈와 오른쪽 눈의 회전 속도가 맞지 않는다.",choices:[
      {id:"desk-inan",owner:"inan",label:"미나의 상태와 발언 타이밍 확인",evidence:"mina-load",secondary:"cool-mina",approach:"이난이 미나의 호흡과 홍채 회전, 발언 직후의 신체 반응을 기록한다.",expectation:"F3 고정 자료 · 미나 부하 완화",result:"정정 발언 종료 직후 체온과 처리 부하가 함께 상승한 기록이 남았다.",quote:"체온 올라갔어요. 손 떨림도 있고요. 미나 선생님, 그건 잉크 아니고 코피예요."},
      {id:"desk-epi",owner:"epi-minos",label:"처음 틀린 말을 한 장면 보존",evidence:"first-statement",approach:"에피가 미나의 최초 발언과 그 직후 움직인 서가를 한 장면으로 묶는다.",expectation:"F3 고정 자료",result:"‘다섯 명’이라는 발언이 끝난 프레임에서 출입 표시가 셋에서 다섯으로 바뀌었다.",quote:"말이 먼저였어. 화면이 바뀐 건 그다음이고. 둘 다 내가 갖고 있어."}
    ]}
  ],
  patterns:[
    {id:"rewrite",label:"재작성",description:"기록의 현재 문장이 더 짙어진다."},
    {id:"source-loss",label:"출처 소실",description:"보호되지 않은 자료의 출처가 잠시 지워진다."},
    {id:"confirmation",label:"확신 편향",description:"가장 오래 남은 오류가 주변 기록을 끌어당긴다."}
  ],
  ...
  abilities:{
    ...
    "kang-unshim":[{id:"trace",verb:"gatherIntel",label:"화제 · 충돌 관측 확산",description:"다음 판단에서 숨은 관측 하나 공개 · 정보 +1.",quote:"제보 캡처 다 올려. 지워진 줄만 모으면 뭐가 끼어든 건지 보이겠지."}],
    ...
    "kim-wooju":[{id:"analyze",verb:"analyze",label:"노이즈 캔슬링 · 원본 분리",description:"단말기 원본/현재 표시 대조 · 다음 오류 예측. 다음 자기 차례까지 방전.",cooldown:"cool",quote:"현재값 껐어요. 원본만 띄웁니다. 미나 선생님은 제 화면 좀 그만 건드리세요."}],
    mageuna:[{id:"rule",verb:"declareRule",label:"마그나 카르타 · 정정 제한",description:"집행 1회를 써 다음 재작성을 차단한다.",ruleCost:1,quote:"승인 도장이 없는 문장은 이 기록지에 들어올 수 없습니다."}],
  ...
  supportAdapters:{
    ...
    "kang-unshim":{verb:"gatherIntel",label:"긴급 제보 대조",description:"다음 판단에 숨은 관측 하나를 추가 공개한다."},
